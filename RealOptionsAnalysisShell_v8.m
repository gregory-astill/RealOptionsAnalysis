...
function trialmat = RealOptionsAnalysisShell_v8(graphs,N,rho,MC,N1,J,subsidy,eps,SIM,aE,aK)
...
if graphs == 1;                     %Turns on/off graph (mc_sigma)
...
for al=0:0               %%%%%%%% TO DO, insert another for loop for subsidies so all EnvR are the same 
...
%          pause;
        for j=1:J
            ...
            if (sim-2)*J+j < 1 | trialmat((sim-2)*J+j,12) == 0 
                ...
                s = mc_sigma_v10(testR, rho, ejbar, alpha, N1, N, MC, graphs, eps);              
                %s = mc_sigma_v10(REnvR, rho, ejbar, alpha, N1, N, MC, graphs);            
...
OO = sprintf('Q%d', alpha*(SIM*J) + SIM*J + trial(1,2))
